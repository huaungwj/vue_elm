<template>
  ...
  <div class="shop_list_container" data-mu-loading-color="secondary" data-mu-loading-overlay-color="rgba(0, 0, 0, 0)" v-loading="loading">
    <!--商家列表-->
    <div class="shoplist_container">
      <!--每一个商家的信息-->
      <ul v-if="shop_list.length">
        <router-link v-for="(shop,index) in shop_list" :key="shop + index" :to="{path: '/shop',query: {'geohash': $store.state.geohash,'id': shop.id}}">
        <li class="shop_list" >
          <!--左边图片-->
          <section class="shop_left">
            <img class="shop_img" :src="`${img_base}${shop.image_path}`" alt="商家">
          </section>
          <!--右边的信息-->
          <hgroup class="shop_right">
            ...
            <header class="shop_detail_header ellipsis ">
              ...
              <ul class="shop_detail_ul">
                <li class="supports" v-for="(support,index) in shop.supports" :key="support + index">
                  {{support.icon_name}}
                </li>
              </ul>
            </header>
            ...
            <div class="shop_center_message">
              <section class="star_message">
                ...
                <span class="order_span">月销{{shop.recent_order_num}}单</span>
              </section>
              <!--快递方式-->
            ...
            <footer class="shop_footer_msg">
              <span class="fee">￥20起送 / {{shop.piecewise_agent_fee.tips}}</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      /*初始化*/
      async initData() {
        let res = await shopList(this.$route.query.geohash.match(/(\S*),/)[1], this.$route.query.geohash.match(/,(\S*)/)[1]);
        this.shop_list = [...res.data]
      },
      //监听父级传来的数据发生变化时，触发此函数重新根据属性值获取数据
      async listenPropChange() {
        let res = await sortShopList(this.latitude, this.longitude, 0, 20, this.restaurantCategoryIds,this.restaurantCategoryId, this.sortByType, this.deliveryMode, this.supportIds,);
