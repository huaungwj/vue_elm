<template>
  ...
  <div style="background-color: #f5f5f5">
    ...
    <head-top>
      ...
      <a style="color: #fff" slot="right" @click="goTo('/login')"
        >登录 | 注册</a
      >
    </head-top>
    <!-- 定位选择 -->
    ...
    <section class="hot_city_content">
      ...
      <ul class="citylistul">
        <li v-for="(city, index) in hot_city" :key="city + index">
          <router-link
            style="color: #3190e8;text-align: center"
            :to="`city/${city.id}`"
          >
            {{ city.name }}
          </router-link>
    ...
    <!--全部的城市列表-->
    <section
      class="group_city_container"
      v-for="(citys, key, index) in sortGroupCity"
      :key="key + index"
    >
    ...
    sortGroupCity() {
      ...
      for (let i = 65; i <= 90; i++) {
        if (this.group_city[String.fromCharCode(i)]) {
          sortCitys[String.fromCharCode(i)] = this.group_city[
